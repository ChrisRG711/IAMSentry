...
def analyze_iam_recommendations(json_file):
    ...
    user_actions = []
    role_usage_analysis = defaultdict(lambda: {
        'users': set(),
        'total_permissions': 0,
        'used_permissions': 0,
        'recommendations': []
    ...
    for rec in recommendations:
        ...
        user_actions.append({
            ...
            'Exercised_Permissions': '; '.join(exercised_perms[:5]) if exercised_perms else 'None'
        })
        ...
        if removed_role != 'N/A':
            ...
            role_usage_analysis[removed_role]['recommendations'].append({
                ...
                'exercised_perms': exercised_perms
            })
...
def create_user_actions_csv(user_actions, filename='iam_recommendations_report.csv'):
    ...
    fieldnames = [
        'Account_Email', 'Account_Type', 'Project', 'Current_Role',
        'Recommendation', 'Description', 'Total_Permissions', 'Used_Permissions',
        'Unused_Permissions', 'Waste_Percentage', 'Risk_Score', 
        'Over_Privilege_Score', 'Priority', 'Exercised_Permissions'
    ]
...
def create_role_analysis_csv(role_analysis, filename='role_analysis_report.csv'):
    ...
    for role, data in role_analysis.items():
        ...
        role_data.append({
            ...
            'Affected_Users': '; '.join(list(data['users'])[:3]) + ('...' if len(data['users']) > 3 else ''),
            ...
            'Custom_Role_Suggestion': f"custom_{role.replace('roles/', '').replace('/', '_')}" if avg_waste > 50 else 'N/A'
        })
    ...
    fieldnames = [
        'Role_Name', 'Affected_Users_Count', 'Affected_Users', 
        'Total_Permissions', 'Max_Used_Permissions', 'Average_Waste_Percentage',
        'Recommendation', 'Top_Used_Permissions', 'Custom_Role_Suggestion'
    ]
...
def main():
    ...
    high_waste_roles = [r for r in role_analysis.values() 
