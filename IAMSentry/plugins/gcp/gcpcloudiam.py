"""Plugin to process the data retrieved from `gcpcloud.IAMSentry` plugin"""

import datetime
import json

""" . = IAMSentry which is the current module, for now it changes, to remove any depedency name incase if there is any changes folder name in the future
"""

from IAMSentry.helpers import hlogging
from IAMSentry.models.applyrecommendationmodel import IAMApplyRecommendationModel
...
class GCPIAMRecommendationProcessor:
    ...
    gcpcloud.IAMSentry IAMRecommendation_record."""
    ...
    def init_blocklist(self, enforcer=None):
        ...
        self._apply_recommendation_blocklist_account_types = enforcer.get(
    ...
    def init_whitelist(self, enforcer=None):
        ...
        self._apply_recommendation_allowlist_account_types = enforcer.get(
    ...
    def init_cloud_resource(self, enforcer=None):
        ...
        self._recommender_resource = util_gcp.build_resource(
            service_name="recommender", key_file_path=self._apply_recommendations_svc_acc_key_file
        )
    ...
    def eval(self, record):
            ...
            dict: Processed record.
                {
                    'GCPIAMProcessor':  {
                            ...
                            'recommendation_recommender_subtype': recommenderSubtype,
                            'recommendation_insights': associatedInsights
                    }
                }
        """
        # Extract the different `IAMSentry_record.recommendation_action.value`
        ...
        if iam_raw_record is not None:
            recommendation_dict.update(
                {
                    ...
                    "recommendation_actions": iam_raw_record["content"]["operationGroups"][0][
                        "operations"
                    ],
                    "recommendation_recommender_subtype": iam_raw_record["recommenderSubtype"],
                    "recommendation_insights": [
                        i.get("insights") for i in iam_raw_record["associatedInsights"]
                    ],
                }
            )
            ...
            for op_grp in iam_raw_record["content"]["operationGroups"]:
                ...
                recommendation_dict.update({"account_type": _actor_type, "account_id": _actor})
            ...
            if insights:
                ...
                if _content:
                    _actor_exercised_permissions = len(
                        _content.get("exercisedPermissions", [])
                    ) + len(_content.get("inferredPermissions", []))
                    _actor_total_permissions = _content.get("currentTotalPermissionsCount", "0")
                    _actor_exercised_permissions_category = insights[0].get("category", "")
            ...
            if _res["raw"]["stateInfo"]["state"] == "SUCCEEDED":
                _res["score"].update({"risk_score": 0, "over_privilege_score": 0})
            ...
            if self._enforcer and _res["raw"]["stateInfo"]["state"] == "ACTIVE":
                _log.info(
                    "ENFORCING#Project:%s,Recommendations:%s",
                    hlogging.obfuscated(recommendation_info[1]),
                    hlogging.obfuscated(recommendation_info[7]),
                )
                ...
                if _recomemndation_applied:
                    ...
                    _res["apply_recommendation"].update(
                        {
                            ...
                            "recommendation_applied_time": str(
                    ...
                    _log.info(
                        "APPLIED#Project:%s,Recommendations:%s",
                        hlogging.obfuscated(recommendation_info[1]),
                        hlogging.obfuscated(recommendation_info[7]),
                    )
                    ...
                    _log.info(
                        "NOT-APPLIED#Project:%s,Recommendations:%s",
                        hlogging.obfuscated(recommendation_info[1]),
                        hlogging.obfuscated(recommendation_info[7]),
                    )
    ...
    def _enforce_recommendation(self, record):
        ...
        if _processor_record and _score_record:
            ...
            flag_blacklist = self._validate_blacklist(
                _project=_project, _account_id=_account_id, _account_type=_account_type
            )

            # Check whitelist validation
            flag_whitelist = self._validate_whitelist(
                _project=_project, _account_type=_account_type
            )

            if flag_blacklist and flag_whitelist:
                # If Recommendation is for SA, apply only for ['REMOVE_ROLE', 'REPLACE_ROLE']
                if _account_type == "serviceAccount" and _processor_record.get(
                    "recommendation_recommender_subtype"
                ) in ["REMOVE_ROLE", "REPLACE_ROLE"]:
                ...
                _we_want_to_apply_recommendation = (
                    _we_want_to_apply_recommendation
                    and self._validate_safety_score(
                        _account_type=_account_type, _safety_score=_safety_score
                    )
                )
            ...
            if _we_want_to_apply_recommendation:
                ...
                _log.info(
                    "SHOULD BE APPLIED TO TARGET=project:%s,account:%s,account_type(%s)",
                    hlogging.obfuscated(_project),
                    hlogging.obfuscated(_account_id),
                    _account_type,
                )
                if self._mode_enforce:
                    _log.info("Executing recommendation in enforce mode")
                    _etag = record.get("raw", {}).get("etag", "")
                    _status = self._execute_recommendation(
                        cloud_resource=cloud_resource,
                        _project=_project,
                        _recommendation_actions=_recommendation_actions,
                        _recommendation_id=_recommendation_id,
                        _etag=_etag,
                    )
                    return _status
                else:
                    _log.info("Scan-only mode - recommendation not applied")
                    return False
    ...
    def _validate_blacklist(
        self, _project, _account_id, _account_type
    ):  # will do the cleanup code later
        if _project in self._apply_recommendation_blocklist_projects:
            _log.warning("Project %s is in excluded list", hlogging.obfuscated(_project))
            return False

        if _account_id in self._apply_recommendation_blocklist_accounts:
            _log.warning("Account %s is in excluded list", hlogging.obfuscated(_account_id))
            return False

        if _account_type in self._apply_recommendation_blocklist_account_types:
            _log.warning("Account type %s is in excluded list", _account_type)
            return False
    ...
    def _validate_safety_score(
        self, _account_type, _safety_score
    ):  # Logic from defining whether we'll remove or not based on the pre-configuration score

        _account_type = _account_type.lower()
        std_min_score = self._apply_recommendation_min_score[_account_type]

        _log.info(
            "GCP Recommendation Safety Score(%s) >= Minimum Score Accepted(%s)",
            _safety_score,
            std_min_score,
        )
    ...
    def _execute_recommendation(
        self, cloud_resource, _project, _recommendation_actions, _recommendation_id, _etag
    ):
        """Execute IAM policy changes based on recommendation.

        Arguments:
            cloud_resource: GCP Cloud Resource Manager API resource.
            _project: Project ID to modify.
            _recommendation_actions: List of IAM actions to perform.
            _recommendation_id: Full recommendation resource name.
            _etag: Recommendation etag for marking as succeeded.

        Returns:
            bool: True if recommendation was successfully applied.
        """
        try:
            _policies = (
                cloud_resource.projects()
                .getIamPolicy(resource=_project, body={"options": {"requestedPolicyVersion": "1"}})
                .execute()
            )
            ...
            for _recommendation_action in _recommendation_actions:
                if _recommendation_action.get("action") == "remove":
                    member = _recommendation_action.get("pathFilters").get(
                        "/iamPolicy/bindings/*/members/*"
                    )
                    role = _recommendation_action.get("pathFilters").get(
                        "/iamPolicy/bindings/*/role"
                    )
                    _updated_policies = self.modify_policy_remove_member(
                        _updated_policies, role, member
                    )
            ...
            status = (
                self._recommender_resource.projects()
                .locations()
                .recommenders()
                .recommendations()
                .markSucceeded(
                    body={
                        "etag": _etag,
                        "stateMetadata": {"reviewed-by": "iamsentry", "owned-by": "security"},
                    },
            ...
            _log.info(
                "Successfully applied recommendation: %s",
                hlogging.obfuscated(_recommendation_id.split("/")[-1]),
            )
            return True
    ...
    def done(self):
        ...
        """
        _log.info(
            "Recommendation applied: %s; Recommendations applied today: %s",
            self._recommendation_applied,
            self._recommendation_applied_today,
        )
