...
class GCPIAMRecommendationProcessor:
        ...
        if _processor_record and _score_record:
            ...
            if flag_blacklist and flag_whitelist:
                ...
                if (
                    ...
                    and _processor_record.get('recommendation_recommender_subtype') in ['REMOVE_ROLE', 'REPLACE_ROLE']
                ):
    ...
    def _execute_recommendation(
        ...
        try:
            ...
            _log.info('Successfully applied recommendation: %s', hlogging.obfuscated(_recommendation_id.split('/')[-1]))
            return True

        except Exception as e:
            _log.error('Failed to execute recommendation %s: %s',
                      hlogging.obfuscated(_recommendation_id.split('/')[-1]), e)
            return False
