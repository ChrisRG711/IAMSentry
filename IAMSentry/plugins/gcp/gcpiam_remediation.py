...
class GCPIAMRemediationProcessor:
    ...
    def _analyze_remediation_options(self, record):
        ...
        remediation_plan = {
            ...
            "priority": self._calculate_remediation_priority(
                waste_percentage, risk_score, account_type
            ),
        }
        ...
        if waste_percentage >= 100:
            ...
            if custom_role_id:
                ...
                remediation_plan["reason"] = (
                    f"Role has {waste_percentage}% waste - migrate to custom role"
                )
            else:
                remediation_plan["recommended_action"] = "review_manual"
                remediation_plan["reason"] = (
                    f"High waste ({waste_percentage}%) but no custom role available"
                )
            ...
            remediation_plan["recommended_action"] = "review_manual"
            remediation_plan["reason"] = (
                f"Moderate waste ({waste_percentage}%) - manual review recommended"
            )
    ...
    def _simulate_remediation(self, plan):
        ...
        if action == "remove_binding":
            ...
            _log.info(
                "SIMULATED: Would remove binding %s from %s",
                plan["current_role"],
                plan["account_id"],
            )
            ...
            _log.info(
                "SIMULATED: Would migrate %s from %s to %s",
                plan["account_id"],
                plan["current_role"],
                custom_role,
            )
