    ...
    for i, account in enumerate(accounts_to_analyze, 1):
        ...
        try:
            ...
            if result['status'] == 'success':
                print(f"✅ {account}: {result['total_api_calls']} API calls, {len(result['permissions_used'])} permissions")
            else:
...
def generate_summary_report(results, failed_accounts, output_file, days_back):
    ...
    with open(output_file, 'w') as f:
        ...
        if successful:
            ...
            active_accounts = sorted(successful, key=lambda x: x['total_api_calls'], reverse=True)[:10]
            for account in active_accounts:
                f.write(f"- **{account['service_account']}**: {account['total_api_calls']} API calls, "
        ...
        for result in sorted(results, key=lambda x: x.get('total_api_calls', 0), reverse=True):
            ...
            if result['status'] == 'success':
                f.write(f"- **Status**: ✅ Active (found {result['log_entries_found']} log entries)\n")
                f.write(f"- **API Calls**: {result['total_api_calls']} total, {result['unique_api_calls']} unique\n")
                f.write(f"- **Services Used**: {', '.join(result['services_used']) if result['services_used'] else 'None'}\n")
